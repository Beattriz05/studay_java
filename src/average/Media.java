package average;

public class Media {
    public static void main (String[] args){
        double nota1 = 8.9;
        double nota2 = 7.5;
        double media1 = (nota1+nota2) /2;
        System.out.printf("Média 1: %.2f%n", media1);
        double media2 = calcularSegundaMedia();
        double media3 = calcularTerceiraMedia();
        double mediaGlobal = (media1 + media2 + media3) / 3;
        System.out.println("---Resultado Final---");
        System.out.printf("Média Global: %.2f%n", mediaGlobal);
    }
    public static double calcularSegundaMedia() {
        double nota1 = -5.8;
        double nota2 = 8.0;
        double media2 = (nota1 + nota2) / 2;
        System.out.printf("Média 2: %.2f%n", media2);
        return media2;
    }
    public static double calcularTerceiraMedia() {
        double nota1 = 5.0;
        double nota2 = 9.0;
        double media3 = (nota1+nota2) / 2;
        System.out.printf("Média 3: %.2f%n", media3);
        return media3;
    }
}